...
import { updateMotorista } from '@/app/_actions/motoristaActions'
...
interface MotoristaEditarFormProps {
  motorista: MotoristaResponseDTO
}

export function MotoristaEditarForm({ motorista }: MotoristaEditarFormProps) {
  ...
  const {
    formatTrim,
    formatUrl,
    formatTelefone: formatTelefoneUtil,
  ...
  const form = useForm<MotoristaUpdateDTO>({
    resolver: zodResolver(updateMotoristaSchema),
    defaultValues: {
      nome: motorista.nome,
      email: motorista.email,
      telefone: formatTelefone(motorista.telefone),
      foto: motorista.foto?.url || ''
    },
    mode: 'onChange'
  })
  ...
  async function onSubmit(data: MotoristaUpdateDTO): Promise<void> {
    const dataToSubmit = {
      ...data,
      foto: data.foto || ''
    }
    ...
    startTransition(async () => {
      try {
        const result = await updateMotorista(motorista.id, dataToSubmit)
        ...
        if (result.success) {
          ...
          const errorMessage =
            result.error ?? 'Ocorreu um erro ao atualizar o motorista.'
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t pt-6">
            <div className="flex gap-4">
              <Button
                type="submit"
                disabled={isPending}
                aria-label="Salvar alterações do motorista"
              >
