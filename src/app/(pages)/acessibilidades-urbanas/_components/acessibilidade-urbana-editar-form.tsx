...
import { updateAcessibilidadeUrbana } from '@/app/_actions/acessibilidadeUrbanaActions'
...
interface AcessibilidadeUrbanaEditarFormProps {
  acessibilidadeUrbana: AcessibilidadeUrbanaResponseDTO
}

export function AcessibilidadeUrbanaEditarForm({
  acessibilidadeUrbana
}: AcessibilidadeUrbanaEditarFormProps) {
  ...
  const {
    formatTrim,
    formatUrl,
    formatTelefone: formatTelefoneHook,
    formatCEP: formatCEPHook,
    formatEstado,
  ...
  const form = useForm<AcessibilidadeUrbanaUpdateDTO>({
    resolver: zodResolver(updateAcessibilidadeUrbanaSchema),
    defaultValues: {
      nome: acessibilidadeUrbana.nome,
      email: acessibilidadeUrbana.email,
      telefone: formatTelefone(acessibilidadeUrbana.telefone),
      categoria: acessibilidadeUrbana.categoria,
      logo: acessibilidadeUrbana.logo?.url || '',
      endereco: {
        logradouro: acessibilidadeUrbana.endereco?.logradouro || '',
        numero: acessibilidadeUrbana.endereco?.numero || '',
        cidade: acessibilidadeUrbana.endereco?.cidade || '',
        bairro: acessibilidadeUrbana.endereco?.bairro || '',
        cep: formatCEP(acessibilidadeUrbana.endereco?.cep || ''),
        estado: acessibilidadeUrbana.endereco?.estado || '',
        pais: acessibilidadeUrbana.endereco?.pais || 'Brasil',
        complemento: acessibilidadeUrbana.endereco?.complemento || ''
      },
      fotos: acessibilidadeUrbana.fotos.map((foto) => foto.url) || [],
      recursos:
        acessibilidadeUrbana.recursos.map((r) => ({
          simbolo: r.simbolo,
  ...
  async function onSubmit(data: AcessibilidadeUrbanaUpdateDTO): Promise<void> {
    const dataToSubmit = {
      ...data,
      logo: data.logo || '',
      fotos: data.fotos || [],
    ...
    startTransition(async () => {
      try {
        const result = await updateAcessibilidadeUrbana(
        ...
        if (result.success) {
          notifySuccess({
            message: 'Acessibilidade urbana atualizada com sucesso!'
          })
          router.push(APP_ROUTES.ACESSIBILIDADE_URBANA_LISTAR())
        } else {
          const errorMessage =
            result.error ??
            'Ocorreu um erro ao atualizar a acessibilidade urbana.'
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        {...createOnChangeFormatHandler(field, formatCEPHook)}
                        value={field.value}
          ...
          <div className="flex gap-4 border-t pt-6">
            <Button
              type="submit"
              disabled={isPending}
              aria-label="Salvar alterações da acessibilidade urbana"
            >
