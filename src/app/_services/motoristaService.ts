import 'server-only'

import { apiClient } from '@/app/_api/apiClient'
import { ApiError } from '@/app/_api/errorHandler'
import {
  API_ROUTES,
  CACHE_CONFIG,
  CACHE_TAGS
} from '@/app/_constants/appSettingsConstants'
import type {
  ComentarioResponseDTO,
  MotoristaResponseDTO
} from '@/app/_dtos/response'
import type { ApiResponse } from '../_types/apiResponsesType'

export async function getMotoristas(): Promise<MotoristaResponseDTO[]> {
  const result = await apiClient<ApiResponse<MotoristaResponseDTO[]>>(
    API_ROUTES.MOTORISTA,
    {
      next: {
        revalidate: CACHE_CONFIG.REVALIDATE_SHORT,
        tags: [CACHE_TAGS.MOTORISTAS]
      }
    }
  )

  return result?.data || []
}

export async function getMotoristaById(
  id: string
): Promise<MotoristaResponseDTO | null> {
  try {
    const result = await apiClient<ApiResponse<MotoristaResponseDTO>>(
      `${API_ROUTES.MOTORISTA}/${id}`,
      {
        next: {
          revalidate: CACHE_CONFIG.REVALIDATE_SHORT,
          tags: [CACHE_TAGS.MOTORISTAS, CACHE_TAGS.MOTORISTA_ID(id)]
        }
      }
    )

    return result?.data || null
  } catch (error) {
    if (error instanceof ApiError && error.status === 404) {
      return null
    }

    throw error
  }
}

export async function getMotoristaComentarios(
  id: string
): Promise<ComentarioResponseDTO[]> {
  try {
    const result = await apiClient<ApiResponse<ComentarioResponseDTO[]>>(
      `${API_ROUTES.MOTORISTA}/${id}/comentarios`,
      {
        next: {
          revalidate: CACHE_CONFIG.REVALIDATE_COMMENTS,
          tags: [CACHE_TAGS.COMENTARIOS, CACHE_TAGS.MOTORISTA_COMENTARIOS(id)]
        }
      }
    )

    return result?.data || []
  } catch (error) {
    if (error instanceof ApiError && error.status === 404) {
      return []
    }

    throw error
  }
}
